...
void Interface::run() {
    ...
    while (true) {
        int check;
        std::cout << "1. Вывести первое число ( голову очереди )\n2. Существует ли элемент в очереди" << std::endl;
        ...
        try {
            ...
            case 2:
                if (!queue.isEmpty()) {
                    std::cout << "Элемент существует, очередь не пустая" << std::endl;
                } else {
                    std::cout << "Элемент не найден, очередь пустая" << std::endl;
                }
                break;
            case 3:
                queue.pop();
